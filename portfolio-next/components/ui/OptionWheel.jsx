...
const OptionWheel = forwardRef(({
  ...
  const applyTarget = useCallback(
    ...
    // Move useImperativeHandle here so it has access to applyTarget
    useImperativeHandle(ref, () => ({
      setTarget: (value) => applyTarget(value, false)
    }), [applyTarget]);

    // Wheel / touchpad scrolling, registered manually so it can be non-passive.
  useEffect(() => {
    const el = rootRef.current;
    if (!el || disableWheelScroll) return;
    const onWheel = e => {
      e.preventDefault();
      const cfg = cfgRef.current;
  ...
  }, [applyTarget, disableWheelScroll]);
